...
{
	unsigned long long			lenght_base;
...
void	ft_cond(char c, va_list args, int *nbc)
{
	...
	if (c == 's')
		*nbc += ft_putstr(va_arg(args, const char *));
	if (c == 'd' || c == 'i')
		ft_putnbr(va_arg(args, int), nbc);
	if (c == 'c')
		*nbc += ft_putchar(va_arg(args, int));
	if (c == 'x')
		ft_putnbr_base(va_arg(args, unsigned int), hexalower, nbc);
	if (c == 'X')
		ft_putnbr_base(va_arg(args, unsigned int), hexaupper, nbc);
	if (c == 'u')
		ft_putnbr_base(va_arg(args, unsigned int), "0123456789", nbc);
	if (c == 'p')
